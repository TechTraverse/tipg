...
logger = logging.getLogger("wlfs_tipg.rds")
...
def rds_connect_args(pg_settings: PostgresSettings, rds_settings: RDSSettings) -> tuple[PostgresSettings, dict]:
    ...
    port = int(get_ssm_param(rds_settings.port_ssm_param) or pg_settings.postgres_port or 5432)
    ...
    passwd = None if rds_settings.use_iam_auth else (
        get_secret(rds_settings.pass_secret_id) or pg_settings.postgres_pass
    )
    ...
    if rds_settings.use_iam_auth:
        ...
        kwargs["password"] = functools.partial(get_rds_token, host, port, user, region)
...
def get_rds_token(host: str|None, port: int|None, user: str|None, region: str|None) -> str:
    logger.debug(f"Retrieving RDS IAM token with host: {host}, port: {port}, user: {user}, region: {region}")
...
def get_ssm_param(name: str|None) -> str|None:
    if not name:
        return None
...
def get_secret(id: str|None) -> str|None:
    if not id:
        return None
